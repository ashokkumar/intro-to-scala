...
/**
  * These exercises will teach you how to work with the `Vector` data structure in Scala.
  */
object LinearCollectionExercises2 {
  ...
  /**
    ...
    * Hint: same as Seq
    */
  def prepend[A](x: A, xs: Vector[A]): Vector[A] = ???
  ...
  /**
    ...
    */
  def fetch[A](index: Int, xs: Vector[A]): A = ???
  ...
  def isEmpty[A](xs: Vector[A]): Boolean = ???
  }
